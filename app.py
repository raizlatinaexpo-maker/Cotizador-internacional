...
def obtener_moneda(pais):
    if pais in ["Francia", "España", "Países Bajos"]:
        return "EUR", "€"
    else:
        return "USD", "$"
...
# PDF PRO
# =========================
def generar_pdf_bytes(nombre, direccion, pais, ciudad, telefono, email,
                      valor_productos, envio, total, moneda, simbolo):
    ...
    amarillo = colors.HexColor("#FFD60A")
    ...
    elementos.append(
        Paragraph(
            "COTIZACIÓN ENVÍO INTERNACIONAL",
            estilo_subtitulo
        )
    )
    ...
    tabla_info = Table([
        [Paragraph(from_data, estilo_normal),
         Paragraph(to_data, estilo_normal)]
    ], colWidths=[270, 270])
...
if tipo == "Cliente":
    ...
    with col2:
        ...
        valor_productos = st.number_input(
            "💰 Valor productos (USD)",
            min_value=0.0
        )
...
if cotizar_btn:
    ...
    if resultado:
        ...
        moneda, simbolo = obtener_moneda(pais)
        ...
        col3.metric("💵 Tarifa/caja", f"{simbolo}{tarifa:,.2f}")
        col4.metric("🚚 Total envío", f"{simbolo}{total_envio:,.2f}")
